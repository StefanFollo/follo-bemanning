...
import {
  Users, TriangleAlert, X, Key, Building2, Wrench, Search, Eye, ClipboardList,
  ChevronUp, ChevronDown, CircleCheck, Check, Mail, UserRound, Trash2, Info,
  Loader, Download, Stethoscope, Undo2,
} from 'lucide-react';
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 22, color: '#1e3a5f' }}><IkonTekst ikon={Users} size={20} gap={8}>Brukerstyring</IkonTekst></h2>
      ...
      {inviteUrl && (
        <div style={{ background: '#fef9c3', border: '1px solid #fbbf24', borderRadius: 8, padding: '12px 16px', marginBottom: 16, fontSize: 13 }}>
          <strong><IkonTekst ikon={TriangleAlert} size={14} gap={4}>E-post ikke sendt</IkonTekst></strong> — Domenet er ikke verifisert i Resend ennå.<br />
        ...
        <div className="modal-backdrop" onClick={() => setShowForm(false)}>
          <div className="modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn-icon" onClick={() => setShowForm(false)}><Ikon ikon={X} size={16} /></button>
            </div>
            <form onSubmit={handleCreate} style={{ padding: '16px 0 0' }}>
              ...
              <div className="form-group">
                ...
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>
                  {form.role === 'admin' ? <IkonTekst ikon={Key} size={13} gap={4}>Kan se og endre alt</IkonTekst>
      ...
      <div style={{ marginBottom: 20, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 10, overflow: 'hidden' }}>
        <button
          ...
          <span><IkonTekst ikon={ClipboardList} size={16}>Inviter fra ansattliste {inviterbareAnsatte.length > 0 && <span style={{ fontSize: 13, fontWeight: 400, color: '#0369a1' }}>({inviterbareAnsatte.length} uten brukerkonto)</span>}</IkonTekst></span>
          <Ikon ikon={visAnsattInvite ? ChevronUp : ChevronDown} size={16} />
        </button>
          ...
          <div style={{ padding: '0 18px 16px' }}>
            {inviterbareAnsatte.length === 0 ? (
              <div style={{ fontSize: 13, color: '#0369a1', padding: '8px 0' }}>
                <IkonTekst ikon={CircleCheck} size={14} farge="#15803d">Alle ansatte med e-post har allerede brukerkonto.</IkonTekst>
              </div>
            ) : (
              <>
                    ...
                    return (
                      <div key={a.id} style={{ background: 'white', border: '1px solid #e0f2fe', borderRadius: 8, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                        ...
                        <button
                          ...
                          {sender ? 'Sender…' : sendt ? <IkonTekst ikon={Check} size={14} gap={4}>Sendt</IkonTekst> : <IkonTekst ikon={Mail} size={14} gap={4}>Inviter</IkonTekst>}
                        </button>
                      </div>
                    );
                  })}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 48, color: '#6b7280' }}>
          <TomIkon ikon={UserRound} size={40} />
      ...
      <div style={{ marginTop: 32, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 10, padding: '16px 20px', fontSize: 13, color: '#0c4a6e' }}>
        <strong><IkonTekst ikon={Info} size={14} gap={4}>Slik fungerer tilgangene:</IkonTekst></strong>
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            >
              {manuellBackupJobber === 'nedlasting' ? <IkonTekst ikon={Loader} size={14} gap={4}>Lager kopi…</IkonTekst> : <IkonTekst ikon={Download} size={14} gap={4}>Last ned kopi nå</IkonTekst>}
            </button>
            <button
              className="btn-secondary"
              style={{ fontSize: 13 }}
            ...
            >
              {manuellBackupJobber === 'epost' ? <IkonTekst ikon={Loader} size={14} gap={4}>Sender…</IkonTekst> : <IkonTekst ikon={Mail} size={14} gap={4}>Send på e-post</IkonTekst>}
            </button>
            <button
              className="btn-secondary"
              style={{ fontSize: 13 }}
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ margin: 0, fontSize: 17, color: '#1e3a5f' }}><IkonTekst ikon={Stethoscope} size={16}>Diagnose lagring</IkonTekst></h3>
          <button
            className="btn-secondary"
            style={{ fontSize: 13 }}
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px', fontSize: 13 }}>
                ...
                {r.nattbackup?.feil && <div style={{ color: '#dc2626', marginTop: 6 }}><IkonTekst ikon={TriangleAlert} size={13} gap={4}>{r.nattbackup.feil}</IkonTekst></div>}
              </div>
              ...
              {!harMangler && !harInnholdstap && (
                <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: 8, padding: '10px 16px', fontSize: 14, color: '#166534' }}>
                  <IkonTekst ikon={CircleCheck} size={15} farge="#15803d">Ingen data mangler sammenlignet med nattens backup</IkonTekst>
                </div>
              )}

              {harMangler && (
                <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 8, padding: '12px 16px', fontSize: 13 }}>
                  <div style={{ fontWeight: 500, color: '#991b1b', marginBottom: 8 }}><IkonTekst ikon={TriangleAlert} size={14} gap={4}>Elementer som fantes i natt men mangler nå:</IkonTekst></div>
                  ...
                  >
                    {gjenoppretter ? <IkonTekst ikon={Loader} size={14} gap={4}>Gjenoppretter…</IkonTekst> : <IkonTekst ikon={Undo2} size={14} gap={4}>Gjenopprett manglende fra nattens backup</IkonTekst>}
                  </button>
              ...
              {harInnholdstap && (
                <div style={{ background: '#fff7ed', border: '1px solid #fdba74', borderRadius: 8, padding: '12px 16px', fontSize: 13 }}>
                  <div style={{ fontWeight: 500, color: '#9a3412', marginBottom: 8 }}><IkonTekst ikon={TriangleAlert} size={14} gap={4}>Prosjekter som har mistet framdrift/sjekklister siden natten:</IkonTekst></div>
                  {innholdstap.map(p => (
